...
class RingBuffer {
  ...
  public:
    ...
    inline bool isEmpty() const noexcept {
        return tailCounter_.load(std::memory_order_relaxed)
               == headCounter_.load(std::memory_order_acquire);
    };
    ...
    inline std::size_t occupancy() const noexcept {
        return headCounter_.load(std::memory_order_acquire)
               - tailCounter_.load(std::memory_order_acquire);
    };
    ...
    static_assert(sizeof(std::size_t) >= 8
                      || (((std::numeric_limits<std::size_t>::max() - N + 1U) % N == 0U)),
...
inline bool RingBuffer<T, N>::push(const T &value) noexcept {
    ...
    const bool nonFull = (cachedTailCounter_ != headLoopAround)
                         || ((cachedTailCounter_ = tailCounter_.load(std::memory_order_acquire))
                             != headLoopAround);
    if (nonFull) {
        data_[head % N] = value;
        advanceHead();
...
inline bool RingBuffer<T, N>::push(InputIt first, InputIt last) noexcept {
    ...
    if constexpr ((sizeof(std::size_t) >= 8)
                  && (N <= ((std::numeric_limits<std::size_t>::max() / 2) + 1U))) {
        const std::size_t diff = head - N + numElements;
        enoughSpace
            = (cachedTailCounter_ >= diff)
        ...
        enoughSpace
            = ((head - cachedTailCounter_ <= diff)
               || ((head - (cachedTailCounter_ = tailCounter_.load(std::memory_order_acquire)))
                   <= diff));
    }
